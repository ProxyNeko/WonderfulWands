package com.mcmoddev.wonderfulwands.common.util;
...
public abstract class RayTrace {
    ...
    public static RayTraceResult rayTraceBlocksAndEntities(World world, double maxRange, EntityLivingBase source) {
        ...
        List<Entity> entities = world.getEntitiesWithinAABBExcludingEntity(source, aoi);
        double closestDistSqr = Double.MAX_VALUE;
        Entity closestEntity = null;
        for (Entity entity : entities) {
            double distSqr = entity.getDistanceSq(source.posX, source.posY, source.posZ);
            if (distSqr < rangeSqr) {
                // entity is within range
                AxisAlignedBB box = entity.getEntityBoundingBox();
                if (rayIntersectsBoundingBox(rayOrigin, rayDirection, box)) {
                    // entity is in cross-hairs
                    if (distSqr < closestDistSqr) {
                        // entity is closest entity in line of fire
        ...
        if (closestEntity == null) {
            return world.rayTraceBlocks(rayOrigin, rayOrigin.add(mul(rayDirection, maxRange)), true, false, false);
        } else {
